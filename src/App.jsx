...
// ─── Mock Store (acts like a database in memory) ──────────────────────────────
const generatePin = () => Math.floor(1000 + Math.random() * 9000).toString();

const INITIAL_SUBSCRIPTIONS = [
  { id: 1, pin: "1234", name: "Alex Johnson",  email: "alex@email.com",  plan: "Pro",   expiry: "2026-12-31", status: "Active", broker: "Interactive Brokers", copyRatio: 0.5,  pnlToday: +122.75, totalPnl: +1840.20, role: "client" },
  { id: 2, pin: "5678", name: "Sarah Mills",   email: "sarah@email.com", plan: "Elite", expiry: "2026-09-15", status: "Active", broker: "Binance",              copyRatio: 1.0,  pnlToday: +245.50, totalPnl: +3240.80, role: "client" },
  { id: 3, pin: "3344", name: "Mark Davis",    email: "mark@email.com",  plan: "Pro",   expiry: "2026-08-01", status: "Paused", broker: "OANDA",                copyRatio: 0.75, pnlToday: 0,       totalPnl: +980.40,  role: "client" },
  { id: 4, pin: "7788", name: "Liu Wei",       email: "liu@email.com",   plan: "Elite", expiry: "2027-01-10", status: "Active", broker: "Saxo Bank",            copyRatio: 2.0,  pnlToday: +491.00, totalPnl: +6120.60, role: "client" },
];
...
function useAlpacaOrders() {
  ...
  useEffect(() => {
    const load = async () => {
      const data = await alpacaFetch("/orders?status=all&limit=20&direction=desc");
...
// ─── Live Price Seeds (simulated WebSocket ticks) ─────────────────────────────
const PRICE_SEEDS = {
  ...
  "XAU/USD":  { price: 2318.5, spread: 0.30,   step: 2.5,    type: "Metal"     },
...
const EQUITY_CURVE = [
  { date: "Jun 5",  equity: 10312 },
  { date: "Jun 6",  equity: 10167 },
  { date: "Jun 7",  equity: 10656 },
  { date: "Jun 8",  equity: 10859 },
  { date: "Jun 9",  equity: 11537 },
  { date: "Jun 10", equity: 11448 },
  { date: "Jun 11", equity: 11982 },
  { date: "Jun 12", equity: 12273 },
  { date: "Jun 13", equity: 12718 },
  { date: "Jun 14", equity: 13027 },
];

// ─── NOWPayments & Subscription Config ───────────────────────────────────────
const NOWPAYMENTS_API_KEY = "EDJB26R-M5248SA-PPZMNXQ-ZQM2QBD";
const NOWPAYMENTS_IPN_KEY = "81Akcbxv0UW71KxtxbC5UH9aANT2UEnA"; // Replace with your real key
const SUBSCRIPTION_PLANS = [
  {
    id: "starter", name: "Starter", price: 10, currency: "USDTTRC20",
    durationDays: 5, color: "#6b7694",
    features: ["1 broker connection", "Forex & Crypto only", "Standard copy ratio (max 1x)", "Email support"],
  },
  {
    id: "pro", name: "Pro", price: 10, currency: "USDTTRC20",
    durationDays: 5, color: "#4d9dff",
    features: ["3 broker connections", "All 12 instruments", "Copy ratio up to 3x", "Priority support"],
  },
  {
    id: "elite", name: "Elite", price: 10, currency: "USDTTRC20",
    durationDays: 5, color: "#ffd14d",
    features: ["Unlimited brokers", "All 12 instruments", "Copy ratio up to 5x", "Dedicated support", "Custom risk settings"],
  },
];
...
function Badge({ label, color }) {
  const map = {
    green:  { bg: "#00d4aa18", text: "#00d4aa", border: "#00d4aa40" },
    red:    { bg: "#ff4d6d18", text: "#ff4d6d", border: "#ff4d6d40" },
    blue:   { bg: "#4d9dff18", text: "#4d9dff", border: "#4d9dff40" },
    yellow: { bg: "#ffd14d18", text: "#ffd14d", border: "#ffd14d40" },
    purple: { bg: "#b07aff18", text: "#b07aff", border: "#b07aff40" },
    gray:   { bg: "#6b769418", text: "#6b7694", border: "#6b769440" },
  };
...
function useLivePrices() {
  const [prices, setPrices] = useState(() =>
    Object.fromEntries(Object.entries(PRICE_SEEDS).map(([k, v]) => [k, { ...v, prev: v.price, flash: null }]))
  );
  useEffect(() => {
    const id = setInterval(() => {
      setPrices(prev => {
        ...
        Object.keys(next).forEach(sym => {
          const s = next[sym];
          const delta = (Math.random() - 0.49) * s.step;
          const newPrice = Math.max(s.price * 0.9, s.price + delta);
          next[sym] = { ...s, prev: s.price, price: newPrice, flash: delta > 0 ? "up" : "down" };
        });
        return next;
      });
      setTimeout(() => setPrices(p => {
        const n = { ...p };
        Object.keys(n).forEach(k => { n[k] = { ...n[k], flash: null }; });
        return n;
      }), 400);
    }, 1800);
    return () => clearInterval(id);
...
function LiveTicker({ prices, isMobile }) {
  ...
  const TickerItem = ({ sym }) => {
    const p = prices[sym];
    const change = p.price - PRICE_SEEDS[sym].price;
    const pct = (change / PRICE_SEEDS[sym].price) * 100;
    const up = change >= 0;
    const decimals = p.price > 1000 ? 2 : p.price > 10 ? 3 : 4;
    const flashColor = p.flash === "up" ? "#00d4aa" : p.flash === "down" ? "#ff4d6d" : null;
    return (
      <div style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "0 20px", borderRight: "1px solid #1a1f2e", height: "100%" }}>
        <span style={{ color: "#6b7694", fontSize: 11, fontWeight: 700, letterSpacing: "0.04em" }}>{sym}</span>
...
function SubscribePage({ onBack, onPaymentSuccess }) {
  ...
  const [step, setStep]       = useState("plans");    // plans → details → paying → confirming → done
  ...
  const [countdown, setCountdown] = useState(300); // 5 min payment window
  const intervalRef = useRef(null);

  // Countdown timer during payment
  useEffect(() => {
    if (step === "paying") {
    ...
    try {
      const res = await fetch("https://api.nowpayments.io/v1/payment", {
        ...
        body: JSON.stringify({
          ...
          order_id: `GAINPICKER-${Date.now()}`,
          order_description: `GainPicker ${selectedPlan.name} Plan - 5 Days`,
        }),
      });
      ...
      if (data.payment_id) {
        ...
        // Simulate for demo if API key not yet set
        setPaymentData({
          payment_id: "DEMO-" + Date.now(),
          pay_address: "TRX_DEMO_ADDRESS_" + Math.random().toString(36).slice(2, 10).toUpperCase(),
      ...
      setPaymentData({
        payment_id: "DEMO-" + Date.now(),
        pay_address: "TDemo7xR9kPqW3mNvBsLtYuHgFjCdAeZi",
        pay_amount: selectedPlan.price,
        pay_currency: "USDT (TRC20)",
        payment_status: "waiting",
      });
  ...
  const checkPayment = async () => {
    setChecking(true);
    try {
      const res = await fetch(`https://api.nowpayments.io/v1/payment/${paymentData.payment_id}`, {
      ...
      if (data.payment_status === "finished" || data.payment_status === "confirmed") {
        ...
        // For demo purposes, simulate confirmed after manual check
        setStep("confirming");
  ...
  const completeSubscription = () => {
    ...
    const expiry = addDays(selectedPlan.durationDays);
    const newClient = {
      id: Date.now(), pin, name: form.name, email: form.email,
      plan: selectedPlan.name, expiry, status: "Active",
      broker: "", copyRatio: 1.0, pnlToday: 0, totalPnl: 0, role: "client",
    };
    setPinGenerated(pin);
    setStep("done");
    onPaymentSuccess(newClient);
  };

  const cardStyle = (plan) => ({
    background: selectedPlan?.id === plan.id ? `${plan.color}11` : "#1a1f2e",
    border: `2px solid ${selectedPlan?.id === plan.id ? plan.color : "#252d3d"}`,
    borderRadius: 14, padding: isMobile ? 16 : 22, cursor: "pointer",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d1117", fontFamily: "'Inter',-apple-system,sans-serif", color: "#f0f4ff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: isMobile ? 16 : 24 }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        ...
        <div style={{ color: "#f0f4ff", fontSize: 22, fontWeight: 800, letterSpacing: "-0.02em" }}>GainPicker</div>
      ...
      <div style={{ width: "100%", maxWidth: 640 }}>
        ...
        {step === "plans" && (
          <>
              ...
              {SUBSCRIPTION_PLANS.map(plan => (
                <div key={plan.id} onClick={() => setSelectedPlan(plan)} style={cardStyle(plan)}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                    ...
                    {selectedPlan?.id === plan.id && <span style={{ color: plan.color, fontSize: 16 }}>✓</span>}
                  </div>
                  ...
                  <div style={{ borderTop: `1px solid ${plan.color}33`, marginTop: 12, paddingTop: 12 }}>
                    {plan.features.map(f => (
                      <div key={f} style={{ color: "#8892aa", fontSize: 12, marginBottom: 6, display: "flex", gap: 7, alignItems: "flex-start" }}>
                        <span style={{ color: plan.color, fontWeight: 700, marginTop: 1 }}>✓</span> {f}
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <button onClick={() => selectedPlan && setStep("details")} disabled={!selectedPlan}
              style={{ width: "100%", background: selectedPlan ? "linear-gradient(135deg,#4d9dff,#7b5ea7)" : "#252d3d", border: "none", borderRadius: 10, padding: 14, color: selectedPlan ? "#fff" : "#6b7694", fontSize: 15, fontWeight: 700, cursor: selectedPlan ? "pointer" : "not-allowed" }}>
              Continue with {selectedPlan?.name || "a Plan"} →
            </button>
            <div style={{ textAlign: "center", marginTop: 14 }}>
              <button onClick={onBack} style={{ background: "none", border: "none", color: "#6b7694", fontSize: 13, cursor: "pointer" }}>← Back to Login</button>
            </div>
          </>
        )}
          ...
          <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 18, padding: isMobile ? 20 : 32 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Your full name"
                style={{ width: "100%", background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 8, padding: "12px 14px", color: "#f0f4ff", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
            </div>
            ...
            <button onClick={createPayment} style={{ width: "100%", background: "linear-gradient(135deg,#4d9dff,#7b5ea7)", border: "none", borderRadius: 10, padding: 14, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", marginBottom: 10 }}>
              Proceed to Payment →
            </button>
        ...
        {step === "paying" && paymentData && (
          <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 18, padding: isMobile ? 20 : 32 }}>
            ...
            <div style={{ background: "#1a1f2e", border: "2px solid #00d4aa44", borderRadius: 12, padding: 18, marginBottom: 16, textAlign: "center" }}>
              ...
              <div style={{ color: "#00d4aa", fontSize: 36, fontWeight: 800, letterSpacing: "-0.02em" }}>{paymentData.pay_amount} USDT</div>
            ...
            <div style={{ background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 12, padding: 16, marginBottom: 16 }}>
              ...
              <div style={{ background: "#0d1117", borderRadius: 8, padding: "12px 14px", fontFamily: "monospace", fontSize: isMobile ? 11 : 13, color: "#4d9dff", wordBreak: "break-all", border: "1px solid #1f2535", marginBottom: 8 }}>
                {paymentData.pay_address}
              </div>
              <button onClick={() => navigator.clipboard?.writeText(paymentData.pay_address)}
                style={{ background: "#252d3d", border: "none", borderRadius: 6, color: "#8892aa", padding: "6px 14px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                📋 Copy Address
              </button>
            </div>
            ...
            <div style={{ background: "#ffd14d0d", border: "1px solid #ffd14d33", borderRadius: 10, padding: "12px 14px", marginBottom: 20 }}>
              ...
              <div style={{ color: "#8892aa", fontSize: 12, lineHeight: 1.6 }}>
                ...
                • Do NOT send from an exchange that doesn't support TRC20<br />
            ...
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              <div style={{ color: countdown < 60 ? "#ff4d6d" : "#6b7694", fontSize: 13 }}>
                Payment window expires in <span style={{ fontWeight: 700, fontFamily: "monospace", fontSize: 16, color: countdown < 60 ? "#ff4d6d" : "#ffd14d" }}>{fmtCountdown(countdown)}</span>
              </div>
            </div>

            <button onClick={checkPayment} disabled={checking}
              style={{ width: "100%", background: checking ? "#252d3d" : "linear-gradient(135deg,#00d4aa,#00a884)", border: "none", borderRadius: 10, padding: 14, color: checking ? "#6b7694" : "#fff", fontSize: 15, fontWeight: 700, cursor: checking ? "not-allowed" : "pointer", marginBottom: 10 }}>
        ...
        {step === "confirming" && (
          <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 18, padding: 40, textAlign: "center" }}>
            ...
            <div style={{ color: "#6b7694", fontSize: 13 }}>Verifying your transaction on the blockchain. This takes just a moment.</div>
        ...
        {step === "done" && (
          <div style={{ background: "#141824", border: "1px solid #00d4aa44", borderRadius: 18, padding: isMobile ? 24 : 40, textAlign: "center" }}>
            ...
            <button onClick={onBack} style={{ width: "100%", background: "linear-gradient(135deg,#4d9dff,#7b5ea7)", border: "none", borderRadius: 10, padding: 14, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
              Login with PIN →
            </button>
          </div>
        )}
...
function LoginScreen({ onLogin, subscriptions, onSubscribe }) {
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      ...
      if (sub) { onLogin(sub); }
      else { setError("Invalid PIN. Please check your subscription PIN."); setLoading(false); }
    }, 700);
  };

  return (
    <div style={{ minHeight: "100vh", background: "#0d1117", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Inter',-apple-system,sans-serif", padding: 20 }}>
      ...
      <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 20, padding: isMobile ? 24 : 36, width: "100%", maxWidth: 380, boxShadow: "0 24px 80px #00000099" }}>
        ...
        <input type="password" value={pin} onChange={e => setPin(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()}
          placeholder="••••" maxLength={8}
          style={{ width: "100%", background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 10, padding: "14px 16px", color: "#f0f4ff", fontSize: 22, letterSpacing: "0.4em", outline: "none", boxSizing: "border-box", fontFamily: "monospace", marginBottom: 10 }} />
        ...
        <button onClick={handleLogin} disabled={loading || pin.length < 4}
          style={{ width: "100%", background: pin.length < 4 ? "#252d3d" : "linear-gradient(135deg,#4d9dff,#7b5ea7)", border: "none", borderRadius: 10, padding: 14, color: pin.length < 4 ? "#6b7694" : "#fff", fontSize: 15, fontWeight: 700, cursor: pin.length < 4 ? "not-allowed" : "pointer", marginTop: 4 }}>
        ...
        <div style={{ borderTop: "1px solid #1f2535", marginTop: 20, paddingTop: 16, textAlign: "center" }}>
          ...
          <button onClick={onSubscribe} style={{ width: "100%", background: "#00d4aa18", border: "1px solid #00d4aa44", borderRadius: 10, padding: "11px", color: "#00d4aa", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
            Subscribe Now — $10 USDT →
          </button>
        </div>
        <div style={{ color: "#2a3040", fontSize: 10, textAlign: "center", marginTop: 14 }}>
          Demo: <span style={{ color: "#2a3855" }}>9999</span> Master · <span style={{ color: "#2a3855" }}>1234</span> / <span style={{ color: "#2a3855" }}>5678</span> Client
        </div>
      </div>
    </div>
  );
...
function ClientSetupModal({ onSave, onClose, existing }) {
  const [form, setForm] = useState(existing || { broker: "", apiKey: "", apiSecret: "", riskPercent: 1, copyRatio: 1.0, maxLoss: 500, assets: [], lotSizeMode: "proportional" });
    ...
    <div style={{ position: "fixed", inset: 0, background: "#000000aa", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: isMobile ? 12 : 20 }}>
      <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 20, padding: isMobile ? 20 : 32, width: "100%", maxWidth: 540, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 32px 100px #000000cc" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            <div style={{ color: "#f0f4ff", fontSize: 18, fontWeight: 700 }}>CopyTrade Setup</div>
            <div style={{ color: "#6b7694", fontSize: 12, marginTop: 2 }}>Connect your broker & configure risk settings</div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12, marginBottom: 20 }}>
          <div>
            ...
            <input value={form.apiKey} onChange={e => update("apiKey", e.target.value)} placeholder="Your broker API key" style={inp} />
          </div>
          <div>
            <label style={lbl}>API SECRET</label>
            <input type="password" value={form.apiSecret} onChange={e => update("apiSecret", e.target.value)} placeholder="••••••••••••" style={inp} />
          </div>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
                ...
                <div key={name} onClick={() => toggleAsset(name)} style={{ border: `1px solid ${on ? "#4d9dff" : "#252d3d"}`, background: on ? "#4d9dff0d" : "#1a1f2e", borderRadius: 10, padding: "11px 12px", cursor: "pointer", display: "flex", gap: 9, alignItems: "flex-start", transition: "all 0.15s" }}>
                  ...
                  <div>
                    <div style={{ display: "flex", gap: 5, alignItems: "center", marginBottom: 2 }}>
                      ...
                      <span style={{ color: on ? "#4d9dff" : "#f0f4ff", fontWeight: 700, fontSize: 12 }}>{name}</span>
                    </div>
          ...
          {form.assets.length > 0 && <div style={{ marginTop: 8, background: "#0d1117", borderRadius: 7, padding: "7px 12px", border: "1px solid #1f2535" }}>
            <span style={{ color: "#4b5568", fontSize: 10 }}>Selected ({form.assets.length}): </span>
            <span style={{ color: "#4d9dff", fontSize: 10, fontWeight: 600 }}>{form.assets.join(" · ")}</span>
          </div>}
        </div>
        ...
        <div style={{ background: "#1a1f2e", borderRadius: 12, padding: 18, marginBottom: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: 12 }}>
            <div>
              <label style={lbl}>COPY RATIO</label>
              <input type="number" value={form.copyRatio} step={0.25} min={0.25} max={5} onChange={e => update("copyRatio", parseFloat(e.target.value))} style={inp} />
              <div style={{ color: "#4b5568", fontSize: 10, marginTop: 4 }}>0.5x = half lot size</div>
            </div>
            <div>
              <label style={lbl}>RISK/TRADE (%)</label>
              <input type="number" value={form.riskPercent} step={0.5} min={0.5} max={10} onChange={e => update("riskPercent", parseFloat(e.target.value))} style={inp} />
              <div style={{ color: "#4b5568", fontSize: 10, marginTop: 4 }}>% of account balance</div>
            </div>
            <div>
              <label style={lbl}>MAX DAILY LOSS ($)</label>
              <input type="number" value={form.maxLoss} step={50} min={50} onChange={e => update("maxLoss", parseFloat(e.target.value))} style={inp} />
              <div style={{ color: "#4b5568", fontSize: 10, marginTop: 4 }}>Pauses copying at limit</div>
            </div>
          </div>
        </div>
              ...
              return (
                <div key={value} onClick={() => update("lotSizeMode", value)} style={{ border: `1px solid ${on ? "#00d4aa" : "#252d3d"}`, background: on ? "#00d4aa0a" : "#1a1f2e", borderRadius: 11, padding: "14px 16px", cursor: "pointer", transition: "all 0.15s" }}>
                  <div style={{ display: "flex", gap: 9, alignItems: "center", marginBottom: 6 }}>
                    <div style={{ width: 15, height: 15, borderRadius: "50%", border: `2px solid ${on ? "#00d4aa" : "#3a4258"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      {on && <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#00d4aa" }} />}
                    </div>
                    <span style={{ fontSize: 14 }}>{icon}</span>
                    <span style={{ color: on ? "#00d4aa" : "#f0f4ff", fontWeight: 700, fontSize: 13 }}>{title}</span>
                  </div>
                  <div style={{ color: "#8892aa", fontSize: 12, lineHeight: 1.5, marginLeft: 24, marginBottom: 7 }}>{desc}</div>
                  <div style={{ marginLeft: 24, background: "#0d1117", borderRadius: 6, padding: "5px 10px", color: on ? "#00d4aa" : "#4b5568", fontSize: 11, fontFamily: "monospace", border: `1px solid ${on ? "#00d4aa33" : "#1f2535"}` }}>
                    {example}
                  </div>
                </div>
              );
...
// ─── PIN Generator Modal (Master only) ───────────────────────────────────────
function PinModal({ onClose, onAdd }) {
  ...
  const handleGenerate = () => {
    const pin = generatePin();
    const newClient = { id: Date.now(), pin, ...form, status: "Active", copyRatio: 1.0, pnlToday: 0, totalPnl: 0, role: "client" };
    setGenerated(pin);
    onAdd(newClient);
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000aa", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: isMobile ? 12 : 20 }}>
      <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 20, padding: isMobile ? 20 : 32, width: "100%", maxWidth: 440, boxShadow: "0 32px 100px #000000cc" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            <div style={{ color: "#f0f4ff", fontSize: 18, fontWeight: 700 }}>Generate Subscription PIN</div>
            <div style={{ color: "#6b7694", fontSize: 12, marginTop: 2 }}>Create access PIN for a new client</div>
          </div>
          <button onClick={onClose} style={{ background: "#252d3d", border: "none", color: "#6b7694", borderRadius: 8, width: 34, height: 34, cursor: "pointer", fontSize: 18 }}>×</button>
        </div>
          ...
          <>
            <div style={{ display: "grid", gap: 14, marginBottom: 20 }}>
              <div>
                ...
                <input value={form.name} onChange={e => update("name", e.target.value)} placeholder="e.g. John Smith" style={inp} />
              </div>
              <div>
                <label style={lbl}>CLIENT EMAIL</label>
                <input type="email" value={form.email} onChange={e => update("email", e.target.value)} placeholder="client@email.com" style={inp} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={lbl}>PLAN</label>
                  <select value={form.plan} onChange={e => update("plan", e.target.value)} style={{ ...inp, cursor: "pointer" }}>
                ...
                <div>
                  ...
                  <input type="date" value={form.expiry} onChange={e => update("expiry", e.target.value)} style={inp} />
                </div>
              </div>
            </div>
            ...
            <div style={{ background: "#1a1f2e", borderRadius: 10, padding: 14, marginBottom: 20 }}>
              ...
              {[
                { plan: "Starter", color: "#6b7694", features: ["1 broker", "Forex & Crypto only", "Email support"] },
                { plan: "Pro",     color: "#4d9dff", features: ["3 brokers", "All instruments", "Priority support"] },
                { plan: "Elite",   color: "#ffd14d", features: ["Unlimited brokers", "All instruments", "Dedicated support", "Custom ratio up to 5x"] },
              ].map(p => (
            ...
            <button onClick={handleGenerate} disabled={!form.name || !form.email || !form.expiry}
              style={{ width: "100%", background: (!form.name || !form.email || !form.expiry) ? "#252d3d" : "linear-gradient(135deg,#4d9dff,#7b5ea7)", border: "none", borderRadius: 10, padding: 14, color: (!form.name || !form.email || !form.expiry) ? "#6b7694" : "#fff", fontSize: 15, fontWeight: 700, cursor: (!form.name || !form.email || !form.expiry) ? "not-allowed" : "pointer" }}>
              Generate PIN & Add Client
            </button>
          </>
        ) : (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ background: "#1a1f2e", borderRadius: 10, padding: 14, marginBottom: 20, textAlign: "left" }}>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                <Badge label={form.plan} color={form.plan === "Elite" ? "yellow" : "blue"} />
                <span style={{ color: "#4b5568", fontSize: 11 }}>Expires {form.expiry}</span>
              </div>
            </div>
            <div style={{ background: "#ffd14d11", border: "1px solid #ffd14d33", borderRadius: 8, padding: "10px 14px", marginBottom: 20 }}>
              <span style={{ color: "#ffd14d", fontSize: 12 }}>⚠ Send this PIN securely. It grants full dashboard access.</span>
            </div>
...
function Header({ user, onLogout, rightSlot, isMobile }) {
  ...
  return (
    <div style={{ background: "#141824", borderBottom: "1px solid #1f2535", padding: isMobile ? "0 16px" : "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 58, position: "relative" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {rightSlot}
        {!isMobile && <div style={{ textAlign: "right", marginLeft: 6 }}>
          <div style={{ color: "#f0f4ff", fontSize: 13, fontWeight: 600 }}>{user.name}</div>
          <div style={{ color: "#6b7694", fontSize: 11 }}>{user.role === "master" ? "Master Trader" : user.broker || "Client"}</div>
        </div>}
        <button onClick={onLogout} style={{ background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 8, color: "#6b7694", padding: isMobile ? "7px 10px" : "7px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
          {isMobile ? "⏻" : "Logout"}
        </button>
      </div>
    </div>
...
function MobileTabBar({ tabs, active, onChange }) {
  return (
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#141824", borderTop: "1px solid #1f2535", display: "flex", zIndex: 100, paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
      {tabs.map(t => (
...
function MasterDashboard({ user, onLogout, subscriptions, onAddClient, prices }) {
  ...
  const todayPnl    = account ? parseFloat(account.unrealized_pl)  : 309;
  const totalPnl    = account ? parseFloat(account.unrealized_plpc) * 100 : 30.3;

  // Map Alpaca positions to display format
  const liveTrades = positions.length > 0 ? positions.map(p => ({
    id:      p.asset_id,
    asset:   p.symbol,
    type:    p.asset_class === "crypto" ? "Crypto" : "Stock",
  ...
  })) : Object.entries(prices).slice(0, 6).map(([sym, p], i) => ({
    id: i + 1, asset: sym, type: p.type, side: i % 3 === 1 ? "SELL" : "BUY",
    entry: PRICE_SEEDS[sym].price, current: p.price,
    size: [1.5, 0.05, 10, 0.8, 0.2, 5][i] || 1,
    pnl: (p.price - PRICE_SEEDS[sym].price) * ([1.5, 0.05, 10, 0.8, 0.2, 5][i] || 1) * (i % 3 === 1 ? -1 : 1),
    status: "OPEN", date: new Date().toISOString().split("T")[0],
  }));

  // Map Alpaca orders to display format
  const recentOrders = orders.length > 0 ? orders.map(o => ({
    id:     o.id,
    asset:  o.symbol,
    type:   o.asset_class === "crypto" ? "Crypto" : "Stock",
    side:   o.side.toUpperCase(),
    size:   parseFloat(o.qty || o.filled_qty || 0),
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d1117", fontFamily: "'Inter',-apple-system,sans-serif", color: "#f0f4ff", paddingBottom: isMobile ? 70 : 0 }}>
      ...
      <Header user={user} onLogout={onLogout} isMobile={isMobile}
        rightSlot={
          <button onClick={() => setShowPin(true)} style={{ background: "linear-gradient(135deg,#4d9dff,#7b5ea7)", border: "none", borderRadius: 8, color: "#fff", padding: isMobile ? "7px 10px" : "7px 14px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
            {isMobile ? "＋ PIN" : "＋ Generate PIN"}
          </button>
        }
      />
      ...
      {!isMobile && (
        <div style={{ background: "#141824", borderBottom: "1px solid #1f2535", padding: "0 24px", display: "flex", gap: 2 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ background: "none", border: "none", padding: "14px 16px", color: tab === t.id ? "#4d9dff" : "#6b7694", borderBottom: tab === t.id ? "2px solid #4d9dff" : "2px solid transparent", cursor: "pointer", fontWeight: 600, fontSize: 13, fontFamily: "inherit" }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding: isMobile ? 14 : 24, maxWidth: 1200, margin: "0 auto" }}>
        ...
        {tab === "overview" && (
          <>
            ...
            <div style={{ background: account ? "#00d4aa0a" : "#ffd14d0a", border: `1px solid ${account ? "#00d4aa33" : "#ffd14d33"}`, borderRadius: 10, padding: "10px 16px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ color: account ? "#00d4aa" : "#ffd14d", fontSize: 12, fontWeight: 700 }}>
                {account ? `✓ Alpaca Paper Account Connected — ${account.account_number}` : "⟳ Connecting to Alpaca Paper Trading…"}
              </div>
              {account && <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#00d4aa" }} />
                <span style={{ color: "#00d4aa", fontSize: 10, fontWeight: 700 }}>LIVE</span>
              </div>}
            </div>

            <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
              <StatCard small={isMobile} label="Today's P&L" value={fmtMoney(todayPnl)} color={pnlColor(todayPnl)} sub="Unrealized" />
              <StatCard small={isMobile} label="Open Positions" value={positions.length || liveTrades.length} sub="Active" />
            ...
            <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, padding: isMobile ? 14 : 20, marginBottom: 16 }}>
              ...
              <ResponsiveContainer width="100%" height={isMobile ? 160 : 200}>
                <AreaChart data={EQUITY_CURVE}>
                  <defs><linearGradient id="eq" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#4d9dff" stopOpacity={0.25} /><stop offset="95%" stopColor="#4d9dff" stopOpacity={0} /></linearGradient></defs>
                  <CartesianGrid stroke="#1f2535" vertical={false} />
                  <XAxis dataKey="date" tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => `$${(v / 1000).toFixed(1)}k`} width={44} />
                  <Tooltip contentStyle={{ background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 10, fontSize: 12 }} formatter={v => [`$${v.toLocaleString()}`, "Equity"]} />
                  <Area type="monotone" dataKey="equity" stroke="#4d9dff" strokeWidth={2.5} fill="url(#eq)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, padding: isMobile ? 14 : 20 }}>
              <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 16 }}>Daily P&L</div>
              <ResponsiveContainer width="100%" height={isMobile ? 140 : 180}>
                <BarChart data={DAILY_PERFORMANCE}>
                  <CartesianGrid stroke="#1f2535" vertical={false} />
                  <XAxis dataKey="date" tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => `$${v}`} width={40} />
                  <Tooltip contentStyle={{ background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 10, fontSize: 12 }} formatter={v => [`$${v}`, "P&L"]} />
                  <Bar dataKey="pnl" fill="#00d4aa" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap: 10 }}>
              {Object.entries(prices).map(([sym, p]) => {
                ...
                const pct = (change / PRICE_SEEDS[sym].price) * 100;
                const up = change >= 0;
                const dec = p.price > 1000 ? 2 : p.price > 10 ? 3 : 4;
                const flashBg = p.flash === "up" ? "#00d4aa0a" : p.flash === "down" ? "#ff4d6d0a" : "transparent";
                return (
                  <div key={sym} style={{ background: "#141824", border: `1px solid ${up ? "#00d4aa33" : "#ff4d6d33"}`, borderRadius: 12, padding: "16px 14px", transition: "background 0.3s", background: flashBg || "#141824" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                      <div>
                        <div style={{ color: "#f0f4ff", fontWeight: 700, fontSize: 14 }}>{sym}</div>
                        <div style={{ color: "#6b7694", fontSize: 10, marginTop: 2 }}>{p.type}</div>
                      </div>
                      <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#00d4aa", marginTop: 4 }} />
                    </div>
                    <div style={{ color: up ? "#00d4aa" : "#ff4d6d", fontSize: 20, fontWeight: 800, fontFamily: "monospace", letterSpacing: "-0.02em" }}>{fmt(p.price, dec)}</div>
                    <div style={{ color: up ? "#00d4aa" : "#ff4d6d", fontSize: 12, marginTop: 4 }}>
                      {up ? "▲" : "▼"} {fmt(Math.abs(change), dec)} ({Math.abs(pct).toFixed(2)}%)
                    </div>
            ...
            <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, overflow: "hidden", marginTop: 6 }}>
              <div style={{ padding: "14px 18px", borderBottom: "1px solid #1f2535", fontWeight: 700, fontSize: 14 }}>
                Open Positions (Live P&L) {positions.length > 0 && <span style={{ color: "#00d4aa", fontSize: 11, marginLeft: 8 }}>● {positions.length} from Alpaca</span>}
              </div>
              {liveTrades.length === 0 ? (
                <div style={{ padding: 24, textAlign: "center", color: "#4b5568", fontSize: 13 }}>No open positions right now</div>
              ) : liveTrades.map((t, i) => (
                <div key={t.id} style={{ padding: "12px 18px", borderTop: i === 0 ? "none" : "1px solid #1a1f2e", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    <Badge label={t.side} color={t.side === "BUY" ? "green" : "red"} />
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 13 }}>{t.asset}</div>
                      <div style={{ color: "#6b7694", fontSize: 10 }}>{t.type} · {t.size} shares</div>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ color: pnlColor(t.pnl), fontWeight: 700, fontSize: 14 }}>{fmtMoney(t.pnl)}</div>
                    <div style={{ color: "#4b5568", fontSize: 10 }}>Entry: ${fmt(t.entry, 2)}</div>
                  </div>
                </div>
              ))}
            </div>
                ...
                {positions.map(p => {
                  ...
                  return (
                    <div key={p.asset_id} style={{ background: "#141824", border: `1px solid ${pnlColor(pnl)}33`, borderRadius: 12, padding: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                        <div>
                          <div style={{ fontWeight: 800, fontSize: 16 }}>{p.symbol}</div>
                          <div style={{ color: "#6b7694", fontSize: 11 }}>{p.asset_class?.toUpperCase()} · {qty > 0 ? "LONG" : "SHORT"} {Math.abs(qty)} shares</div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ color: pnlColor(pnl), fontWeight: 800, fontSize: 18 }}>{fmtMoney(pnl)}</div>
                          <div style={{ color: pnlColor(pnl), fontSize: 11 }}>{pnlPct >= 0 ? "+" : ""}{pnlPct.toFixed(2)}%</div>
                        </div>
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
                        {[
                          { l: "Entry Price",   v: `$${fmt(entry, 2)}` },
                          { l: "Current Price", v: <span style={{ color: pnlColor(curr - entry) }}>${fmt(curr, 2)}</span> },
                          { l: "Market Value",  v: `$${fmt(mktVal, 2)}` },
                          { l: "Side",          v: <Badge label={qty > 0 ? "LONG" : "SHORT"} color={qty > 0 ? "green" : "red"} /> },
                        ].map(({ l, v }) => (
                          <div key={l} style={{ background: "#1a1f2e", borderRadius: 8, padding: "8px 10px" }}>
                            <div style={{ color: "#4b5568", fontSize: 9, marginBottom: 3 }}>{l}</div>
        ...
        {/* ORDERS TAB — Real Alpaca */}
        {tab === "orders" && (
          <div>
            <div style={{ color: "#f0f4ff", fontWeight: 700, fontSize: 15, marginBottom: 14 }}>
              Recent Orders <span style={{ color: "#4b5568", fontSize: 12, fontWeight: 400 }}>— Last 20 from Alpaca</span>
            </div>
            {recentOrders.length === 0 ? (
              <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, padding: 40, textAlign: "center" }}>
                <div style={{ fontSize: 32, marginBottom: 12 }}>📋</div>
                <div style={{ color: "#6b7694", fontSize: 14 }}>No recent orders found</div>
              </div>
            ) : (
                  ...
                  return (
                    <div key={o.id} style={{ padding: "13px 18px", borderTop: i === 0 ? "none" : "1px solid #1a1f2e", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, background: i % 2 === 1 ? "#0f1420" : "transparent" }}>
                      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13 }}>{o.asset}</div>
                          <div style={{ color: "#4b5568", fontSize: 10 }}>{o.date} {o.time}</div>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ color: "#8892aa", fontSize: 12 }}>{o.size} shares</div>
                          <div style={{ color: "#f0f4ff", fontWeight: 600, fontSize: 12 }}>${fmt(o.price, 2)}</div>
                        </div>
                        <Badge label={o.status?.toUpperCase()} color={statusColor} />
                      </div>
                    </div>
                  );
                })}
                  ...
                  <div key={c.id} style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 12, padding: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                      <div>
                        ...
                        <div style={{ color: "#6b7694", fontSize: 11 }}>{c.email}</div>
                      </div>
                      <Badge label={c.status} color={c.status === "Active" ? "green" : "yellow"} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div style={{ background: "#1a1f2e", borderRadius: 8, padding: "8px 10px" }}>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>Today</div>
                        <div style={{ color: pnlColor(c.pnlToday), fontWeight: 700, fontSize: 13 }}>{fmtMoney(c.pnlToday)}</div>
                      </div>
                      <div style={{ background: "#1a1f2e", borderRadius: 8, padding: "8px 10px" }}>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>Total</div>
                        <div style={{ color: pnlColor(c.totalPnl), fontWeight: 700, fontSize: 13 }}>{fmtMoney(c.totalPnl)}</div>
                      </div>
                      <div style={{ background: "#1a1f2e", borderRadius: 8, padding: "8px 10px" }}>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>Broker</div>
                        <div style={{ color: "#8892aa", fontSize: 11, fontWeight: 600 }}>{c.broker || "—"}</div>
                      </div>
                      <div style={{ background: "#1a1f2e", borderRadius: 8, padding: "8px 10px" }}>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>Ratio</div>
                        <div style={{ color: "#4d9dff", fontWeight: 700, fontSize: 13 }}>{c.copyRatio}x</div>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                      <Badge label={c.plan} color={c.plan === "Elite" ? "yellow" : "blue"} />
                      <span style={{ color: "#4b5568", fontSize: 10 }}>PIN: {c.pin} · Exp {c.expiry}</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "#1a1f2e" }}>
                      {["Client", "Plan", "Broker", "Ratio", "Today P&L", "Total P&L", "Status"].map(h => (
                        <th key={h} style={{ padding: "11px 14px", textAlign: "left", color: "#6b7694", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {clients.map((c, i) => (
                      <tr key={c.id} style={{ borderTop: "1px solid #1f2535", background: i % 2 === 1 ? "#0f1420" : "transparent" }}>
                        <td style={{ padding: "13px 14px" }}>
                          <div style={{ fontWeight: 700, fontSize: 13 }}>{c.name}</div>
                          <div style={{ color: "#4b5568", fontSize: 11 }}>{c.email}</div>
                        </td>
                        <td style={{ padding: "13px 14px" }}><Badge label={c.plan} color={c.plan === "Elite" ? "yellow" : "blue"} /></td>
                        <td style={{ padding: "13px 14px", color: "#8892aa", fontSize: 12 }}>{c.broker || "—"}</td>
                        <td style={{ padding: "13px 14px", color: "#4d9dff", fontWeight: 700 }}>{c.copyRatio}x</td>
                        <td style={{ padding: "13px 14px", fontWeight: 700, color: pnlColor(c.pnlToday) }}>{fmtMoney(c.pnlToday)}</td>
                        <td style={{ padding: "13px 14px", fontWeight: 700, color: pnlColor(c.totalPnl) }}>{fmtMoney(c.totalPnl)}</td>
                        <td style={{ padding: "13px 14px" }}><Badge label={c.status} color={c.status === "Active" ? "green" : "yellow"} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {clients.map(c => {
                ...
                return (
                  <div key={c.id} style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 12, padding: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 14 }}>{c.name}</div>
                      <div style={{ color: "#6b7694", fontSize: 11, marginTop: 2 }}>{c.email}</div>
                    </div>
                    <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>PIN</div>
                        <div style={{ color: "#4d9dff", fontWeight: 800, fontFamily: "monospace", fontSize: 16, letterSpacing: "0.1em" }}>{c.pin}</div>
                      </div>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>Plan</div>
                        <Badge label={c.plan} color={c.plan === "Elite" ? "yellow" : "blue"} />
                      </div>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>Expires</div>
                        <div style={{ color: expColor, fontWeight: 700, fontSize: 12 }}>{c.expiry} <span style={{ fontSize: 10 }}>({expDays}d)</span></div>
                      </div>
                      <Badge label={c.status} color={c.status === "Active" ? "green" : "yellow"} />
                    </div>
                  </div>
            ...
            <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, padding: isMobile ? 14 : 20, marginBottom: 14 }}>
              ...
              <ResponsiveContainer width="100%" height={isMobile ? 140 : 180}>
                <LineChart data={DAILY_PERFORMANCE}>
                  <CartesianGrid stroke="#1f2535" vertical={false} />
                  <XAxis dataKey="date" tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} domain={[0, 100]} tickFormatter={v => `${v}%`} width={36} />
                  <Tooltip contentStyle={{ background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 10, fontSize: 12 }} formatter={v => [`${v}%`, "Win Rate"]} />
                  <Line type="monotone" dataKey="winRate" stroke="#00d4aa" strokeWidth={2.5} dot={{ fill: "#00d4aa", r: 3 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, overflow: "hidden" }}>
              ...
              {[...DAILY_PERFORMANCE].reverse().map((d, i) => (
                <div key={d.date} style={{ padding: "11px 18px", borderTop: i === 0 ? "none" : "1px solid #1a1f2e", display: "flex", justifyContent: "space-between", alignItems: "center", background: i % 2 === 1 ? "#0f1420" : "transparent" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <div style={{ background: "#1a1f2e", borderRadius: 3, height: 5, width: isMobile ? 50 : 80, overflow: "hidden" }}>
                      <div style={{ background: d.winRate >= 60 ? "#00d4aa" : "#ff4d6d", height: "100%", width: `${d.winRate}%` }} />
                    </div>
                    <span style={{ color: d.winRate >= 60 ? "#00d4aa" : "#ff4d6d", fontSize: 12, fontWeight: 600 }}>{d.winRate}%</span>
                  </div>
...
function ClientDashboard({ user, onLogout, prices }) {
  ...
  const [settings, setSettings] = useState(user.broker ? { broker: user.broker, copyRatio: user.copyRatio, riskPercent: 1, maxLoss: 500, assets: [], lotSizeMode: "proportional" } : null);
  const isMobile = useIsMobile();
  ...
  const LIVE_TRADES = Object.entries(prices).slice(0, 4).map(([sym, p], i) => ({
    id: i + 1, asset: sym, type: p.type, side: i % 3 === 1 ? "SELL" : "BUY",
    entry: PRICE_SEEDS[sym].price, current: p.price, size: [1.5, 0.05, 10, 0.8][i] || 1,
    masterPnl: (p.price - PRICE_SEEDS[sym].price) * ([1.5, 0.05, 10, 0.8][i] || 1) * (i % 3 === 1 ? -1 : 1),
  }));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d1117", fontFamily: "'Inter',-apple-system,sans-serif", color: "#f0f4ff", paddingBottom: isMobile ? 70 : 0 }}>
      ...
      <Header user={user} onLogout={onLogout} isMobile={isMobile}
        rightSlot={
          <button onClick={() => setShowSetup(true)} style={{ background: settings ? "#1a1f2e" : "linear-gradient(135deg,#4d9dff,#7b5ea7)", border: settings ? "1px solid #252d3d" : "none", borderRadius: 8, color: settings ? "#6b7694" : "#fff", padding: isMobile ? "7px 10px" : "7px 12px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
            {isMobile ? "⚙️" : (settings ? "⚙ Edit Setup" : "⚙ Setup CopyTrade")}
          </button>
        }
      />
      <LiveTicker prices={prices} isMobile={isMobile} />
      ...
      {/* Setup banner */}
      {!settings && (
        <div style={{ background: "#4d9dff0d", border: "1px solid #4d9dff33", margin: isMobile ? "12px 14px" : "16px 24px", borderRadius: 12, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div>
            <div style={{ color: "#4d9dff", fontWeight: 700, fontSize: 13 }}>⚡ Setup Required</div>
            <div style={{ color: "#6b7694", fontSize: 12, marginTop: 2 }}>Connect your broker to start copying trades automatically.</div>
          </div>
          <button onClick={() => setShowSetup(true)} style={{ background: "#4d9dff", border: "none", borderRadius: 8, color: "#fff", padding: "8px 14px", cursor: "pointer", fontWeight: 700, fontSize: 12, whiteSpace: "nowrap" }}>Set Up →</button>
        </div>
      )}
      {settings && (
        <div style={{ background: "#00d4aa0a", border: "1px solid #00d4aa33", margin: isMobile ? "12px 14px" : "16px 24px", borderRadius: 12, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ color: "#00d4aa", fontSize: 13, fontWeight: 700 }}>✓ CopyTrading Active — {settings.broker} · {settings.copyRatio}x · Risk {settings.riskPercent}%</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#00d4aa" }} />
            <span style={{ color: "#00d4aa", fontSize: 11, fontWeight: 700 }}>LIVE</span>
          </div>
        </div>
      )}
      ...
      <div style={{ padding: isMobile ? 14 : 24, maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, padding: isMobile ? 14 : 20 }}>
              ...
              <ResponsiveContainer width="100%" height={isMobile ? 150 : 190}>
                <AreaChart data={DAILY_PERFORMANCE.map(d => ({ ...d, myPnl: +(d.pnl * clientData.copyRatio).toFixed(2) }))}>
                  ...
                  <XAxis dataKey="date" tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: "#6b7694", fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => `$${v}`} width={40} />
                  <Tooltip contentStyle={{ background: "#1a1f2e", border: "1px solid #252d3d", borderRadius: 10, fontSize: 12 }} formatter={v => [`$${v}`, "Your P&L"]} />
                  <Area type="monotone" dataKey="myPnl" stroke="#00d4aa" strokeWidth={2.5} fill="url(#mypnl)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </>
        ...
        {tab === "live" && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {LIVE_TRADES.map(t => {
                const yourPnl = t.masterPnl * clientData.copyRatio;
                const yourLot = (t.size * clientData.copyRatio).toFixed(2);
                const dec = t.entry > 100 ? 2 : 4;
                return (
                  <div key={t.id} style={{ background: "#141824", border: `1px solid ${pnlColor(yourPnl)}33`, borderRadius: 12, padding: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 15 }}>{t.asset}</div>
                        <div style={{ color: "#6b7694", fontSize: 11 }}>{t.type}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ color: pnlColor(yourPnl), fontWeight: 800, fontSize: 18 }}>{fmtMoney(yourPnl)}</div>
                        <div style={{ color: "#4b5568", fontSize: 10 }}>Your P&L</div>
                      </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
                      {[
                        { l: "Direction", v: <Badge label={t.side} color={t.side === "BUY" ? "green" : "red"} /> },
                        { l: "Entry", v: fmt(t.entry, dec) },
                        { l: "Current", v: <span style={{ color: pnlColor(t.current - t.entry) }}>{fmt(t.current, dec)}</span> },
                        { l: "Your Lot", v: <span style={{ color: "#4d9dff" }}>{yourLot}</span> },
                      ].map(({ l, v }) => (
                        <div key={l} style={{ background: "#1a1f2e", borderRadius: 8, padding: "8px 10px" }}>
                          <div style={{ color: "#4b5568", fontSize: 9, marginBottom: 3 }}>{l}</div>
                          <div style={{ fontWeight: 600, fontSize: 12 }}>{v}</div>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ color: "#4b5568", fontSize: 10 }}>Master P&L: {fmtMoney(t.masterPnl)}</span>
                        <span style={{ color: "#4b5568", fontSize: 10 }}>COPYING</span>
                      </div>
                      <div style={{ background: "#1a1f2e", borderRadius: 3, height: 4 }}>
                        <div style={{ background: pnlColor(yourPnl), height: "100%", width: `${Math.min(100, Math.abs(yourPnl))}%`, borderRadius: 3, transition: "width 1s" }} />
                      </div>
                    </div>
                  </div>
                );
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, padding: 20 }}>
                ...
                {[["Name", user.name], ["Email", user.email || "—"], ["PIN", user.pin], ["Plan", user.plan], ["Expiry", user.expiry]].map(([l, v]) => (
                  <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #1a1f2e" }}>
                    <span style={{ color: "#6b7694", fontSize: 13 }}>{l}</span>
                    <span style={{ color: "#f0f4ff", fontWeight: 600, fontSize: 13 }}>{v}</span>
                  </div>
                ))}
              </div>
              {settings ? (
                <div style={{ background: "#141824", border: "1px solid #252d3d", borderRadius: 14, padding: 20 }}>
                  ...
                  {[
                    ...
                    ["Copy Ratio", `${settings.copyRatio}x`],
                    ["Risk/Trade", `${settings.riskPercent}%`],
                    ["Max Daily Loss", `$${settings.maxLoss}`],
                    ["Lot Size Mode", settings.lotSizeMode],
                    ["Instruments", settings.assets.length ? settings.assets.join(", ") : "None selected"],
                  ].map(([l, v]) => (
                    <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #1a1f2e" }}>
                      <span style={{ color: "#6b7694", fontSize: 13 }}>{l}</span>
                      <span style={{ color: "#f0f4ff", fontWeight: 600, fontSize: 12, textAlign: "right", maxWidth: "60%" }}>{v}</span>
                    </div>
                  ))}
...
export default function App() {
  const [screen, setScreen]           = useState("login"); // login | subscribe
  ...
  const handlePaymentSuccess = (newClient) => {
    addClient(newClient);
    // After payment done, go back to login so they can use their new PIN
  };
